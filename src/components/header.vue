...
<!-- 顶部导航栏-->
        <div class="header-wrap">
            ...
            <ul class="top-right">
                ...
                <li v-if="usrStatus"><router-link to="/publish">发布</router-link></li>
        ...
        methods:{
            ...
            dologin(){
                if(this.username==''||this.password=='')
                    ...
                    this.$axios.post(config.ajaxUrl+'blog/user/login',{username:this.username,password:this.password})
                        .then((res) => {
                            if(res.status == 200)
            ...
            dosignup(){
                if(this.password==''||this.username==''||this.email=='')
                    ...
                    this.$axios.post(config.ajaxUrl+'blog/user/register',{username:this.username,password:this.password,email:this.email})
                        .then((res) => {
                            localStorage.setItem('userdata',JSON.stringify(res.data));
                            // 跳转到首页
                            this.$router.push({
...
<style scoped lang="scss">
    .header {
        ...
        .logo{
            ...
            font-size: 30px;
            background: -webkit-linear-gradient(to right,#67B26F,#4ca2cd); /* Safari 5.1 - 6.0 */
